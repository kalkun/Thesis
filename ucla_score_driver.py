...
def main(db, csv_out, filter_source=None, **kwargs):
    ...
    order by a.timestamp desc" % (
        "where b.source = '{}' and c.source = '{}'".format(filter_source, filter_source)
        if filter_source is not None else ""
    )

    df = pd.read_sql(query, pc.session.bind)
    print("DF LENGTH:", len(df))
    if not os.path.exists(csv_out):
        print("Output into {}".format(csv_out))
        print("Generating scores...")
        cs.GenerateChoixScores(df, csv_out)
    print("File exists, therefore not generating any new scores, but reusing")
    ...
    if (db):
        if not kwargs['source']:
            raise ValueError("Should provide 'source' explicitly, when writing to DB")
...
if __name__ == '__main__':
        ...
        parser.add_argument(
            "--include-db",
            action = "store_true",
            help="If set, will commit changes to database"
        )
        parser.add_argument(
            "--source",
            help="The source to use when writing labels to the DB",
        )
        parser.add_argument(
            "--filter",
            metavar="SOURCE",
            choices=["UCLA", "google", "bing", "Luca Rossi - ECB"],
        ...
        main(
            args.include_db,
            args.csv_output or PATH_TO_SAVE,
